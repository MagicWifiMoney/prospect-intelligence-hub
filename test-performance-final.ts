...
async function testGetProspects(cookies: string[]): Promise<ResponseTime> {
  ...
  for (let i = 0; i < 5; i++) {
    try {
      const time = await measureResponseTime(`${BASE_URL}/api/prospects`, cookies);
      times.push(time);
...
async function testGetProspectById(cookies: string[]): Promise<ResponseTime> {
  const times: number[] = [];
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        const time = await measureResponseTime(`${BASE_URL}/api/prospects/${prospectId}`, cookies);
        times.push(time);
        console.log(`  Request ${i + 1}: ${time.toFixed(2)}ms`);
...
async function testConcurrentLoad(cookies: string[]) {
...
async function testLargeDataQuery(cookies: string[]) {
...
async function runPerformanceTests() {
  ...
  try {
    cookies = await getAuthenticatedCookies();
  } catch (error) {
  ...
  // Run tests
  const getProspectsResult = await testGetProspects(cookies);
  const postProspectResult = await testPostProspect(cookies);
  const getProspectByIdResult = await testGetProspectById(cookies);
  const concurrentLoadResult = await testConcurrentLoad(cookies);
  const largeDataQueryResult = await testLargeDataQuery(cookies);
