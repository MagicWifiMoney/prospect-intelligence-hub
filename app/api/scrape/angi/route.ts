...
/**
 * POST: Start a new Angi scrape job
 */
export async function POST(request: NextRequest) {
    try {
        ...
        const run = await scrapeAngi(searchQuery, location, maxResults)
        ...
        return NextResponse.json({
            ...
            apifyRunId: run.id,
            status: run.status,
            message: `Angi scrape started for "${searchQuery}" in ${location}`,
        })

    } catch (error) {
        console.error('Error starting Angi scrape:', error)
        return NextResponse.json(
            { error: 'Failed to start Angi scrape' },
            { status: 500 }
        )
    }
}
...
/**
 * GET: Check status of an Angi scrape job
 */
export async function GET(request: NextRequest) {
    try {
        ...
        if (runStatus.status === 'SUCCEEDED') {
            const results = await getApifyDataset(runStatus.datasetId!) as AngiResult[]
            const importResult = await importAngiResults(results, {
        ...
        console.error('Error checking Angi scrape status:', error)
